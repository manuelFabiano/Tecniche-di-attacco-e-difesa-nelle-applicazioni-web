...
if (isset($_GET['id'])) {
    ...
    $id = $_GET['id'];
    /*OPZIONE 1
    // Rimuove i caratteri che potrebbero essere sfruttati per attacchi SQL
    $blacklistedChars = array("'", '"', ";", "--", "/*", "UNION", "AND", "OR", "SELECT", "INSERT", "UPDATE", "DELETE", "DROP", "EXEC", "ALTER");
    $id = str_replace($blacklistedChars, "", $id);
    */
    ...
    <div class="z-10 relative p-5 overflow-visible bg-transparent w-full h-full">
        <div class="flex flex-col justify-center items-center">
            ...
            <div class="bg-gray-200 rounded-lg shadow p-4 mt-5 mb-5 w-2/3">
                ...
                <form action="./submit_comment.php" method="post" onsubmit="sanitizeComment()">
        ...
        <script>
            ...
            function sanitizeComment() {
                ...
                const sanitizedComment = DOMPurify.sanitize(commentTextArea.value); // Utilizza DOMPurify per la sanitizzazione HTML
                commentTextArea.value = sanitizedComment; // Aggiorna il valore del campo del commento con il testo sanitizzato
            }
            //Show Query
